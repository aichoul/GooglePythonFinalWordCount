...
def calculate_frequencies(file_contents):
    ...
    punctuations = '''!()-[]{};:'"\,<>./?@#$%^&*_~''' # a string
    # uninteresting words are stored in a list
    ...
    for character in file_contents:
        if character in punctuations:
             new_file_contents = file_contents.replace(character, " ")
    
    words = new_file_contents.split()
    
    for keyword in words:
        if keyword.isalpha(): 
            if keyword not in uninteresting_words:
    ...
    for word in words:
        ...
        for character in word:
            if character not in punctuations:
                word_wo_punct=word_wo_punct + character
        
        if word_wo_punct not in uninteresting_words:
            word_dict[word_wo_punct]=word_dict.get(word_wo_punct,0) +1
    """
